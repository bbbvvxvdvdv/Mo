...
void live_timer(int seconds) {
    ...
    cout << "\r" << PINK << "Attack Finished\n" << RESET;
}

bool is_expired() {
    time_t now = time(0);
    tm expiry = {};
    expiry.tm_year = 2026 - 1900;
    expiry.tm_mon = 5 - 1;
    expiry.tm_mday = 5;
...
int main(int argc, char* argv[]) {
    if (!check_binary_name(argv[0])) {
        cout << PINK << "BINARY NAME-> Moin\n" << RESET;
        return 0;
    }

    if (is_expired()) {
        cout << PINK << "PAID BINARY BY TO DM ->@MoinOwner\n" << RESET;
        return 0;
    }

    if (argc < 4 || argc > 5) {
        cout << PINK << "Usage: ./Moin <IP> <PORT> <TIME> [THREADS]\n" << RESET;
        return 0;
    }
